            ...
            default:
                ...
                switch(letter) {
                    ...
                    case 'T':
                        ...
                        break;
//                    case 'U':
//                    	fan_PwmSet(int_value);
//                    	break;
//                    case 'V':
//						beep_PwmSet(int_value);
//						break;
                    case 'X':
  ...
  */
  ...
  // [0. Non-specific/common error-checks and miscellaneous setup]:
    ...
    else if (bit_istrue(command_words, bit(ModalGroup_M13))) {
    	if (bit_istrue(value_words, bit(Word_S)))
    	{
    		fan_PwmSet((int)gc_block.values.s);
    		bit_false(value_words, bit(Word_S));
    	}
    }
    else if (bit_istrue(command_words, bit(ModalGroup_M15))) {
		if (bit_istrue(value_words, bit(Word_S)))
		{
			/*保存参数配置*/
			settings.fire_alarm_time_threshold =((int)gc_block.values.s);
